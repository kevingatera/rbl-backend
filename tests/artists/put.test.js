...
describe("PUT /artists", () => {
  ...
  beforeAll(async () => {
    ...
    apiServer.use(async (req, res, next) => {
      // req.requestContext = { authUser: authUser }   
  ...
  describe("With valid artistId & valid data", () => {
    test("should respond with a 200 status code", async () => {
      const response = await request.put(`/api/artists/${singleArtistMock._id}`).send({
        artist: newArtistDetails
      });
  ...
  describe("With valid data & invalid artistId", () => {
    test("should respond with a 404 status code", async () => {
      const response = await request.put(`/api/artists/${(new ObjectID()).toString()}`).send({
        artist: newArtistDetails
      });
  ...
  describe("With valid artistId but invalid data", () => {
